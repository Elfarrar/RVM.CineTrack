...
public class TmdbClient
{
    ...
    public virtual async Task<TmdbSearchResult<TmdbMultiSearchItem>?> SearchMultiAsync(string query, int page = 1, string language = "pt-BR")
    {
    ...
    public virtual async Task<TmdbMovieDetail?> GetMovieDetailAsync(int tmdbId, string language = "pt-BR")
    {
        var url = $"{BaseUrl}/movie/{tmdbId}?language={language}&append_to_response=credits";
        return await GetAsync<TmdbMovieDetail>(url);
    }

    public virtual async Task<TmdbWatchProviderResponse?> GetMovieWatchProvidersAsync(int tmdbId)
    {
    ...
    public virtual async Task<TmdbTvDetail?> GetTvDetailAsync(int tmdbId, string language = "pt-BR")
    {
        var url = $"{BaseUrl}/tv/{tmdbId}?language={language}&append_to_response=credits";
        return await GetAsync<TmdbTvDetail>(url);
    }

    public virtual async Task<TmdbSeasonDetail?> GetSeasonDetailAsync(int tmdbId, int seasonNumber, string language = "pt-BR")
    {
        var url = $"{BaseUrl}/tv/{tmdbId}/season/{seasonNumber}?language={language}";
        return await GetAsync<TmdbSeasonDetail>(url);
    }

    public virtual async Task<TmdbWatchProviderResponse?> GetTvWatchProvidersAsync(int tmdbId)
    {
    ...
    public virtual async Task<TmdbSearchResult<TmdbMultiSearchItem>?> GetTrendingAsync(string timeWindow = "week", string language = "pt-BR")
    {
        var url = $"{BaseUrl}/trending/all/{timeWindow}?language={language}";
        return await GetAsync<TmdbSearchResult<TmdbMultiSearchItem>>(url);
    }

    public virtual async Task<TmdbUpcomingResponse?> GetUpcomingMoviesAsync(string language = "pt-BR", string region = "BR")
    {
        var url = $"{BaseUrl}/movie/upcoming?language={language}&region={region}";
        return await GetAsync<TmdbUpcomingResponse>(url);
    }

    public virtual async Task<TmdbSearchResult<TmdbMultiSearchItem>?> GetOnTheAirTvAsync(string language = "pt-BR")
    {
